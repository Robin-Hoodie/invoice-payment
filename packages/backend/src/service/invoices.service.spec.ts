...
describe("Invoices Service", () => {
  it(`should return the next invoice number as '22002', when passed the year 2022 
and the 'number' of the latest invoice for said year being '1'`, async () => {
    ...
    expect(await getNextInvoiceNumberForYear(2022)).toEqual("#22002");
  });

  it(`should return the next invoice number as '22001', 
when passed the year 2022 and no invoices yet existing for the year '22002'`, async () => {
    mockedGetLatestInvoice.mockResolvedValueOnce(null);
    expect(await getNextInvoiceNumberForYear(2022)).toEqual("#22001");
  });
});
